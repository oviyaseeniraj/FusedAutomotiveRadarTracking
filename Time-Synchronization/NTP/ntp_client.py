...
class KalmanFilter:
    """Simple Kalman filter for time offset estimation with variable network delays"""
    
    def __init__(self, process_variance=1e-5, measurement_variance=1e-3):
        self.process_variance = process_variance  # Q - how much we trust our model
        self.measurement_variance = measurement_variance  # R - measurement noise
        self.estimate = 0.0  # Current offset estimate
        self.error_covariance = 1.0  # P - estimation error
        
    def update(self, measurement, delay):
        """
        Update filter with new measurement
        Adapt measurement variance based on network delay (higher delay = less trust)
        """
        # Adaptive measurement variance based on delay
        adaptive_R = self.measurement_variance * (1 + delay * 100)
        
        # Prediction (assume offset stays constant)
        predicted_estimate = self.estimate
        predicted_covariance = self.error_covariance + self.process_variance
        
        # Update
        kalman_gain = predicted_covariance / (predicted_covariance + adaptive_R)
        self.estimate = predicted_estimate + kalman_gain * (measurement - predicted_estimate)
        self.error_covariance = (1 - kalman_gain) * predicted_covariance
        
        return self.estimate

def sync_with_ntp(server='pool.ntp.org', samples=5, use_kalman=True):
    """
    Synchronize with NTP server using multiple samples and Kalman filtering
    
    Args:
        server: NTP server address
        samples: Number of samples to take
        use_kalman: Use Kalman filtering for better wireless/cellular performance
        
    Returns:
        dict with sync information
    """
    results = []
    kalman = KalmanFilter() if use_kalman else None
    
    print(f"Querying NTP server {server} ({samples} samples{'with Kalman filtering' if use_kalman else ''})...")
    ...
    for i in range(samples):
        try:
            ...
            if kalman:
                filtered_offset = kalman.update(result['offset'], result['delay'])
                print(f"  Sample {i+1}/{samples}: offset={result['offset']*1000:.2f}ms, "
                      f"filtered={filtered_offset*1000:.2f}ms, delay={result['delay']*1000:.2f}ms")
            else:
                print(f"  Sample {i+1}/{samples}: offset={result['offset']*1000:.2f}ms, "
                      f"delay={result['delay']*1000:.2f}ms")
    ...
    if use_kalman and kalman:
        # Use Kalman filtered result
        final_offset = kalman.estimate
    else:
        # Use median of best half
        best_half = results[:max(1, len(results)//2)]
        final_offset = sum(r['offset'] for r in best_half) / len(best_half)
