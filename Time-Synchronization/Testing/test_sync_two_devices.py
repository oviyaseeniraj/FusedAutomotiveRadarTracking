...
def test_two_device_sync(device1_name, device2_name, time_server, samples=10):
    ...
    try:
        ...
        print(f"  Mean RTT:      {device1_stats['mean_rtt']*1000:>8.3f} ms")
    except Exception as e:
    ...
    print()
    print(f"Best case (if Device 2 has similar offset):")
    print(f"  Estimated sync: ~{device1_stats['stdev_offset']*1000*2:.3f} ms")
    print()
    print(f"Target: < 10 ms")
    print(f"Status: {'✓ ON TRACK' if device1_stats['stdev_offset']*1000*2 < 10 else '⚠ NEEDS IMPROVEMENT'}")
    print()
