function [x_mean, y_mean, covariance_matrix] = visualize_range_angle_posterior_monte_carlo(x_opt, detected_centroid_r1, ...
    ...
    title(['Position Posterior, Frame - ', num2str(frame_idx)]);

    xlabel('X(m)');
    ylabel('Y(m)');
    xlim([-sigma_x, sigma_x]);
    ylim([-sigma_y, sigma_y]);
    ...
    x_samples = x_arr_mesh_flat(idx_samples);
    y_samples = y_arr_mesh_flat(idx_samples);
    
    %% Compute the mean manually
    % (Population mean)
    x_mean = sum(x_samples) / N_MC;
    y_mean = sum(y_samples) / N_MC;
    ...
    delta_x = x_samples - x_mean;
    delta_y = y_samples - y_mean;
    
    % Sum of squares and cross-terms (single-threaded sum is fine here)
    sum_XX = sum(delta_x.^2);        % sum of (x - mean_x_est)^2
    sum_YY = sum(delta_y.^2);        % sum of (y - mean_y_est)^2
    sum_XY = sum(delta_x .* delta_y);     % sum of (x - mean_x_est)*(y - mean_y_est)
    
    cov_xx  = sum_XX / N_MC;
    cov_yy  = sum_YY / N_MC;
    cov_xy = sum_XY / N_MC;
    
    covariance_matrix = [cov_xx,  cov_xy;
                         cov_xy, cov_yy];
